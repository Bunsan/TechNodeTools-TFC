...
public class ItemAlloySheetMod extends ItemMetalSheetMod
{
    private String metal;
    public int metalID;

    public ItemAlloySheetMod (int mID)
    {
        super(mID);
        setMaxDamage(0);
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
    {
        ...
        if(!world.isRemote)
        {
            ...
            if (world.getBlock(x, y, z) == ModBlocks.alloySheetMod && isValid(world, x, y, z))
            {
            ...
            else if (world.getBlock(x, y, z) != ModBlocks.alloySheetMod && isValid(world, sides[0] + x, sides[1] + y, sides[2] + z))
            {
                world.setBlock(sides[0] + x, sides[1] + y, sides[2] + z, ModBlocks.alloySheetMod);
    ...
    public boolean isValid(World world, int i, int j, int k)
    {
        ...
        if (block.isAir(world, i, j, k))
            return true;
        if (block == ModBlocks.alloySheetMod && world.getTileEntity(i, j, k) instanceof TEMetalSheet)
        {
