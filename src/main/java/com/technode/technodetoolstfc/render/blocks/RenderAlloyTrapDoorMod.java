...
import com.technode.technodetoolstfc.block.BlockAlloySheetMod;
import com.technode.technodetoolstfc.block.BlockAlloyTrapDoorMod;
import com.technode.technodetoolstfc.core.ModBlocks;
import com.technode.technodetoolstfc.tileentity.TEAlloyTrapDoorMod;
import net.minecraft.block.Block;
import net.minecraft.client.renderer.RenderBlocks;
import net.minecraft.world.IBlockAccess;

public class RenderAlloyTrapDoorMod extends RenderMetalTrapDoorMod
{
    public static boolean render(Block block, int i, int j, int k, RenderBlocks renderer)
    {
        IBlockAccess access = renderer.blockAccess;
        TEAlloyTrapDoorMod te = (TEAlloyTrapDoorMod) access.getTileEntity(i, j, k);
        int side = te.data & 7;
        int hinge = te.data >> 4;
        float f = 0.0625f;
        ...
        if (BlockAlloyTrapDoorMod.isTrapdoorOpen(access.getBlockMetadata(i, j, k)))
        {
            if(hinge == 0)
            {
                switch(side)
        ...
        int hingeID = te.sheetStack != null ? Math.min(((BlockAlloySheetMod) ModBlocks.alloySheetMod).icons.length-1, te.sheetStack.getItemDamage() >> 5) : 0;

        boolean breaking = renderer.overrideBlockTexture != null;

        if ( ! breaking )
            renderer.setOverrideBlockTexture(((BlockAlloySheetMod)ModBlocks.alloySheetMod).icons[hingeID]);
    ...
    public void renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks renderer)
    {
        ...
        int index = Math.min(((BlockAlloySheetMod) ModBlocks.alloySheetMod).icons.length-1, metadata >> 5);
        renderInvBlock(block, ((BlockAlloySheetMod)ModBlocks.alloySheetMod).icons[index], renderer);
